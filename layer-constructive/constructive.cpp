...
double Constructive::probabilityAttraction (InstanceData &data, int i, int j, double C) {
    return data.probability[j];
};

Customers Constructive::KNeighborProbabilityInsertion(InstanceData &data, int K, double C, double minPrize, double minProb) {
    int n = data.size;
    ...
    while (feasibleTour.path.size() < n)
    {
        ...
        for (int i = 0; i < n; i++) 
        {
            if (!visited[i]) 
            {
                double attraction = probabilityAttraction(data, last, i, C);
